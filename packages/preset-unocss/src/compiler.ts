...
import { atRule, decl, optimizeAst, toCss, walk, type AstNode } from './tailwindcss/ast'
...
export class Compiler {
  ...
  async reset(css?: string) {
    const { ast, designSystem } = await parseCss(
      [DEFAULT_THEME, css].filter((value) => value !== undefined).join('\n'),
    )
  ...
  private splitAstNodes(astNodes: AstNode[]): {
    ...
    return {
      keyframes,
      properties,
      theme: [atRule('@layer', 'theme', theme)],
      utilities,
    }
  }
