import { styleRule, walk, WalkAction } from '../tailwindcss/ast'
...
`
  ...
  const themeAst = styleRule(':root, :host', parse(config))

  walk(themeAst.nodes, (child) => {
    ...
    if (child.kind === 'declaration' && child.property.startsWith('--')) {
      theme.add(unescape(child.property), child.value ?? '', ThemeOptions.REFERENCE)
      return
    }
  })
